...
from olist.order_mine import Order
...
class Seller:
    ...
    def get_review_score(self):
        ...
        olist = Olist()
        data = olist.get_data()
        order_reviews=pd.merge(data['order_reviews'],data['orders'],on='order_id',how='inner')[['order_id','review_score']]
        ...
        merged_data=pd.merge(order_reviews, data['order_items'], on='order_id', how='inner')
        merged_data=merged_data.groupby('seller_id')[['dim_is_five_star','dim_is_one_star','review_score']].agg('mean','mean','mean')
        merged_data.reset_index(inplace=True)
        merged_data=merged_data.rename(columns={"dim_is_five_star": "share_of_five_stars", "dim_is_one_star": "share_of_one_stars"})
        return merged_data
